...
function SizeGuideModal({ open, onClose }) {
  return (
    <div className={`size-guide-overlay${open ? " open" : ""}`} hidden={!open} onClick={(e) => {
...
function ProductCard({ product, onAddToCart, onOpenSizeGuide }) {
  ...
  function handleAdd() {
    onAddToCart({
      ...
      priceDisplay: formatPrice(product.price_cents),
